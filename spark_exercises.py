...
def assignment2() -> DataFrame:
  sql = "SELECT education AS education, COUNT(*) AS education_count FROM users GROUP BY education ORDER BY education_count DESC"
  return spark.sql(sql)
  '''
...
def assignment4(location : str) -> DataFrame:
  sql = f'''SELECT AVG(income) AS avg_income, AVG(spending) AS avg_spending FROM users WHERE location = '{location}'
  '''
  return spark.sql(sql)
  '''
